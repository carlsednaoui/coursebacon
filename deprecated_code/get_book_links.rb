@books = Book.all
puts "Original books count: " + @books.count.to_s

@new_books = Array.new
@books.each do |book|
	unless UrlData.find_by_book_id(book.id).present?
		@new_books.push book.id
	end
end

puts "Books count: " + @new_books.count.to_s

@new_books.each do |book|
	puts "Currently working on :" + book.to_s
	
	url = "#{Book.find_by_id(book).url}"
	
	@url_data = UrlData.new
	@url_data.book_id = book
